...
const markAllAsRead = () => {
    router.post('/member/notifications/read-all', {}, {
        preserveScroll: true,
    });
};

const filterChanged = (f) => {
    router.get('/member/notifications', { filter: f === 'all' ? '' : f }, {
        preserveState: true,
        replace: true,
    });
};
    ...
    <MemberLayout>
        <div class="mx-auto max-w-3xl space-y-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-xl font-bold text-slate-950">Notifikasi</h1>
                    <p class="text-sm text-slate-500">Pengumuman dan hebahan daripada pihak koperasi.</p>
                </div>
                <Button
                    type="button"
                    variant="outline"

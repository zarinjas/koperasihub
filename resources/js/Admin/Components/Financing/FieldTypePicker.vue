...
<template>
  <div class="field-type-picker">
    ...
    <div v-if="compact">
      ...
      <div class="space-y-0.5 max-h-64 overflow-y-auto rounded-lg border border-slate-200 p-1">
        <button v-for="type in filteredTypes" :key="type.value" type="button"
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2">
              <span class="text-sm font-medium text-slate-900 leading-snug">{{ type.label }}</span>
              <span v-if="type.isMemberAutofill" class="shrink-0 rounded bg-purple-50 px-1.5 py-0.5 text-[10px] font-medium text-purple-600 leading-none">Auto</span>
            </div>
            <p class="mt-0.5 text-xs text-slate-500 leading-snug line-clamp-1">{{ type.description }}</p>
          </div>
        </button>
      </div>
    </div>

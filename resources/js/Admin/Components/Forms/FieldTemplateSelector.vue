...
import { FIELD_TEMPLATES } from '@/Admin/Helpers/formFieldTypes';
...
<template>
  <div class="rounded-lg border border-slate-200 bg-white">
    <button
      type="button"
      class="flex w-full items-center gap-2 px-3 py-2.5 text-left text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50"
      @click="isOpen = !isOpen"
    >
      ...
      <span class="flex-1">Templat Soalan Pantas</span>
    ...
    <div v-if="isOpen" class="border-t border-slate-100 px-3 pb-3 pt-2">
      <p class="mb-2 text-xs text-slate-500">
        Gunakan template untuk tambah soalan automatik ke dalam bahagian ini.
      </p>
      ...
      <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
        ...
        >
          <div class="space-y-1">
            ...
            <p class="text-xs leading-relaxed text-slate-500">{{ template.description }}</p>
    ...
    <ConfirmDialog
      ...
      :description="`Templat ini akan menambah ${showConfirm?.fieldCount || 0} soalan ke dalam bahagian ini. Teruskan?`"

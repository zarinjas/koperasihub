...
<template>
    ...
    <div v-show="activeTab === 'dokumen'" class="space-y-6">
        ...
        <FormSection title="Dokumen Produk" description="PDF rasmi untuk rujukan ahli. Format PDF sahaja, maksimum 10MB." :columns="1">
            <div class="space-y-4">
                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    <label class="block text-sm font-medium text-slate-900" for="consent-pdf">Dokumen Consent / Persetujuan</label>
                    <input id="consent-pdf" class="mt-2 block w-full text-sm text-slate-700" type="file" accept=".pdf,application/pdf" @input="form.consent_pdf = $event.target.files[0]" />
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <input id="undertaking-pdf" class="mt-2 block w-full text-sm text-slate-700" type="file" accept=".pdf,application/pdf" @input="form.undertaking_pdf = $event.target.files[0]" />
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <input id="guide-pdf" class="mt-2 block w-full text-sm text-slate-700" type="file" accept=".pdf,application/pdf" @input="form.guide_pdf = $event.target.files[0]" />
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <input id="official-form-template-pdf" class="mt-2 block w-full text-sm text-slate-700" type="file" accept=".pdf,application/pdf" @input="form.official_form_template_pdf = $event.target.files[0]" />
            ...
            <div v-if="product?.product_documents?.length" class="rounded-2xl border border-slate-200 bg-white p-4">
                ...
                <div class="mt-3 space-y-2">
                    ...
                    >
                        ...
                        <p class="mt-1">{{ document.file_name }}</p>
                    </div>
                </div>
            </div>
        </FormSection>

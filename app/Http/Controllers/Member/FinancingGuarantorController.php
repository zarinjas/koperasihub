    ...
    public function index(Request $request): Response
    {
        ...
        $requests = $member->financingGuarantorRequests()
            ...
            ->map(fn (FinancingGuarantor $guarantor) => [
                ...
                'applicant_name' => $guarantor->application?->member?->full_name,
                'applicant_member_no' => $guarantor->application?->member?->member_no,
                'product_name' => $guarantor->application?->product?->name,
                'amount_requested' => $guarantor->application ? 'RM ' . number_format((float) $guarantor->application->amount_requested, 2) : null,
                'tenure_months' => $guarantor->application?->tenure_months,
                'purpose' => $guarantor->application?->purpose,
                'submitted_at' => $guarantor->application?->submitted_at?->format('d/m/Y H:i'),

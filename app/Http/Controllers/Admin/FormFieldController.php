...
class FormFieldController extends Controller
{
    ...
    public function store(StoreFormFieldRequest $request, OnlineForm $onlineForm): RedirectResponse
    {
        $this->ensureSameCooperative($onlineForm);

        $section = $onlineForm->sections()->findOrFail($request->integer('form_section_id'));
        $field = $section->fields()->create($this->validatedPayload($request, $onlineForm));
    ...
    public function update(UpdateFormFieldRequest $request, OnlineForm $onlineForm, FormField $field): RedirectResponse
    {
        $this->ensureSameCooperative($onlineForm);
        abort_unless($field->online_form_id === $onlineForm->id, 404);

        $old = $field->toArray();
        $field->update($this->validatedPayload($request, $onlineForm, $field));
